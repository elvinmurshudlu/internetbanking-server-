...
class UserControl {
  ...
  static async findUser(userInformation) {
    ...
    return {
      user: searchResult,
      sessionCode: await this.sessionGenerator(searchResult.id),
    } //!istifadeci tapildi cookie gonder
  ...
  static async addUserTransaction(
    amount,
    fromUserId,
    fromCard,
    toCard,
  ...
  static async setUserCardAvailability(message) {
    await Cards.update(
      { isAvailable: message.value },
      {
        where: {
          id: message.cardNumber,
        },
      }
    )
  }
  ...
  static async moneyTransfer(data) {
    ...
    if (toUserCard) {
      ...
      await Cards.update(
        { amount: (+toUserCard.amount - +data.amount).toString() },
        {
          where: {
    ...
    await Cards.update(
      { amount: (+fromUserCard.amount + +data.amount).toString() },
      {
        where: {
  ...
  static async deleteRecipientById(id) {
    await Recipients.destroy({ where: { id: id } })
  }
}
